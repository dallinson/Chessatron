...
/*
    The code in this file is more-or-less directly taken from Starzix,
    available at https://github.com/zzzzz151/Starzix
*/
...
constexpr static std::array see_test_cases = {
...
TEST(SEETests, TestStaticExchangeEval) {
    for (auto& test : see_test_cases) {
        std::string test_case = std::string(test);
        const auto tokens = splitString(test_case, '|');

        const auto fen = tokens[0];
        const auto uci_move = tokens[1];
