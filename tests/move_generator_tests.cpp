...
TEST(MoveGeneratorTests, TestCorrectMoveCountStartPos) {
    ...
    auto lst = MoveGenerator::generate_legal_moves<MoveGenType::ALL_LEGAL, false>(c, Side::WHITE);
    ASSERT_EQ(lst.len(), 20);
    MoveList m;
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, m);
    ASSERT_EQ(m.len(), 16);
    m = MoveList();
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, m);
    ASSERT_EQ(m.len(), 4);

    lst = MoveGenerator::generate_legal_moves<MoveGenType::ALL_LEGAL, false>(c, Side::BLACK);
    ASSERT_EQ(lst.len(), 20);
    m = MoveList();
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, m);
    ASSERT_EQ(m.len(), 16);
    m = MoveList();
    m = MoveList();
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, m);
    ASSERT_EQ(m.len(), 4);
}
...
TEST(MoveGeneratorTests, TestCorrectMoveCountMaxMoves) {
    ...
    MoveList rook_moves;
    MoveGenerator::generate_moves<PieceTypes::ROOK, MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, rook_moves);
...
TEST(MoveGeneratorTests, TestEnPassant) {
    ...
    MoveList pawn_moves;
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, pawn_moves);
...
TEST(MoveGeneratorTests, TestDoubleCheck) {
    ...
    auto moves = MoveGenerator::generate_legal_moves<MoveGenType::ALL_LEGAL, false>(c, Side::WHITE);
    ASSERT_EQ(moves.len(), 4);
    auto king_moves = MoveGenerator::generate_legal_moves<MoveGenType::ALL_LEGAL, false>(c, Side::WHITE);
...
TEST(MoveGeneratorTests, TestCorrectMoveCountKiwipete) {
    ...
    auto legal_moves = MoveGenerator::generate_legal_moves<MoveGenType::ALL_LEGAL, false>(c, Side::WHITE);
    ...
    MoveGenerator::generate_moves<PieceTypes::BISHOP, MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, bishop_moves);
    ASSERT_EQ(bishop_moves.len(), 11);

    MoveList king_moves;
    MoveGenerator::generate_moves<PieceTypes::KING, MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, king_moves);
    ...
    MoveGenerator::generate_moves<PieceTypes::QUEEN, MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, queen_moves);
    ASSERT_EQ(queen_moves.len(), 9);

    MoveList knight_moves;
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_LEGAL, false>(c, Side::WHITE, knight_moves);
...
TEST(MoveGeneratorTests, TestCorrectMoveCountKiwipeteB2B3) {
    ...
    auto legal_moves = MoveGenerator::generate_legal_moves<MoveGenType::ALL_LEGAL, false>(c, Side::BLACK);

    MoveList rook_moves;
    MoveGenerator::generate_moves<PieceTypes::ROOK, MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, rook_moves);
    ASSERT_EQ(rook_moves.len(), 9);

    MoveList bishop_moves;
    MoveGenerator::generate_moves<PieceTypes::BISHOP, MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, bishop_moves);
    ASSERT_EQ(bishop_moves.len(), 8);

    MoveList king_moves;
    MoveGenerator::generate_moves<PieceTypes::KING, MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, king_moves);
    ASSERT_EQ(king_moves.len(), 2);
    ...
    MoveList queen_moves;
    MoveGenerator::generate_moves<PieceTypes::QUEEN, MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, queen_moves);
    ASSERT_EQ(queen_moves.len(), 4);

    MoveList knight_moves;
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, knight_moves);
    ASSERT_EQ(knight_moves.len(), 10);

    MoveList pawn_moves;
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_LEGAL, false>(c, Side::BLACK, pawn_moves);
...
TEST(MoveGeneratorTests, TestCorrectCaptureCount) {
    ...
    auto moves = MoveGenerator::generate_legal_moves<MoveGenType::ALL_LEGAL, false>(c, c.get_side_to_move());
    int captures = 0;
