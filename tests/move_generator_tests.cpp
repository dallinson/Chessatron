...
TEST(MoveGeneratorTests, TestCorrectMoveCountStartPos) {
    ...
    auto lst = MoveGenerator::generate_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE);
    ASSERT_EQ(lst.size(), 20);
    MoveList m;
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE, m);
    ASSERT_EQ(m.size(), 16);
    m = MoveList();
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE, m);
    ASSERT_EQ(m.size(), 4);

    lst = MoveGenerator::generate_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK);
    ASSERT_EQ(lst.size(), 20);
    m = MoveList();
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK, m);
    ASSERT_EQ(m.size(), 16);
    m = MoveList();
    m = MoveList();
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK, m);
    ASSERT_EQ(m.size(), 4);
}
...
TEST(MoveGeneratorTests, TestCorrectMoveCountMaxMoves) {
    ...
    MoveList rook_moves;
    MoveGenerator::generate_moves<PieceTypes::ROOK, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE, rook_moves);
...
TEST(MoveGeneratorTests, TestEnPassant) {
    ...
    MoveList pawn_moves;
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE, pawn_moves);
...
TEST(MoveGeneratorTests, TestCorrectMoveCountKiwipete) {
    ...
    auto legal_moves = MoveGenerator::generate_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE);
    ...
    MoveGenerator::generate_moves<PieceTypes::BISHOP, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE, bishop_moves);
    ...
    MoveGenerator::generate_moves<PieceTypes::QUEEN, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE, queen_moves);
    ASSERT_EQ(queen_moves.size(), 9);

    MoveList knight_moves;
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::WHITE, knight_moves);
...
TEST(MoveGeneratorTests, TestCorrectMoveCountKiwipeteB2B3) {
    ...
    auto legal_moves = MoveGenerator::generate_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK);

    MoveList rook_moves;
    MoveGenerator::generate_moves<PieceTypes::ROOK, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK, rook_moves);
    ASSERT_EQ(rook_moves.size(), 9);

    MoveList bishop_moves;
    MoveGenerator::generate_moves<PieceTypes::BISHOP, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK, bishop_moves);
    ...
    MoveList queen_moves;
    MoveGenerator::generate_moves<PieceTypes::QUEEN, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK, queen_moves);
    ASSERT_EQ(queen_moves.size(), 4);

    MoveList knight_moves;
    MoveGenerator::generate_moves<PieceTypes::KNIGHT, MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK, knight_moves);
    ASSERT_EQ(knight_moves.size(), 10);

    MoveList pawn_moves;
    MoveGenerator::generate_pawn_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, Side::BLACK, pawn_moves);
...
TEST(MoveGeneratorTests, TestCorrectCaptureCount) {
    ...
    auto moves = MoveGenerator::generate_moves<MoveGenType::ALL_PSEUDOLEGAL>(c, c.get_side_to_move());
    int captures = 0;
