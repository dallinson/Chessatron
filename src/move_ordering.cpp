...
void MoveOrdering::reorder_moves(MoveList& moves, const ChessBoard& board, const Move pv_move, bool& found_pv_move, std::array<uint32_t, 8192>& history_table, Score capture_see_threshold) {
    found_pv_move = false;
    for (size_t i = 0; i < moves.len(); i++) {
        ...
        if (moves[i].move.is_capture()) {
            ...
            moves[i].see_ordering_result = Search::static_exchange_evaluation(board, moves[i].move, capture_see_threshold);
