...
void MoveOrdering::reorder_moves(MoveList& moves, const ChessBoard& board, const Move pv_move, std::array<int32_t, 8192>& history_table, Move killer, bool& found_pv_move) {
    found_pv_move = false;
    for (size_t i = 0; i < moves.len(); i++) {
        ...
        if (moves[i].move == pv_move) {
            ...
            const auto src_score = ordering_scores[static_cast<uint8_t>(board.piece_at(moves[i].move.get_src_square()).get_type()) - 1];
            const auto dest_type = moves[i].move.get_move_flags() == MoveFlags::EN_PASSANT_CAPTURE ? PieceTypes::PAWN : board.piece_at(moves[i].move.get_dest_square()).get_type();
