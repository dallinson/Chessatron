...
class HistoryTable {
    private:
        ...
        std::unique_ptr<MDArray<Score, 16384, 2>> minor_corr_hist;
    ...
    public:
        HistoryTable() {
            ...
            minor_corr_hist = std::make_unique<MDArray<Score, 16384, 2>>();
            clear(); 
        };
        ...
        Score corrhist_score(const Position& pos, const Score static_eval) const;
        ...
        void update_corrhist_score(const Position& pos, const Score static_eval, const Score search_score, const int depth);
        void clear() { 
            ...
            std::for_each(minor_corr_hist->begin(), minor_corr_hist->end(), [](auto& arr) { arr.fill(0); });
            main_hist.fill(0); 
        };
};