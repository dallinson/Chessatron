...
TUNABLE_SPECIFIER auto razoring_offset = TUNABLE_INT("razoring_offset", 336, 200, 600);
TUNABLE_SPECIFIER auto razoring_multi = TUNABLE_INT("razoring_multi", 240, 100, 400);
...
TUNABLE_SPECIFIER auto nmp_se_divisor = TUNABLE_INT("nmp_se_divisor", 205, 100, 300);

TUNABLE_SPECIFIER auto iir_depth = TUNABLE_INT("iir_depth", 5, 2, 8);

TUNABLE_SPECIFIER auto non_pv_cutnode_lmr_constant = TUNABLE_INT("non_pv_cutnode_lmr_constant", 1109, 512, 2048);
TUNABLE_SPECIFIER auto in_check_lmr_constant = TUNABLE_INT("in_check_lmr_constant", -803, -2048, -512);
TUNABLE_SPECIFIER auto not_improving_lmr_constant = TUNABLE_INT("not_improving_lmr_constant", 1141, 512, 2048);
TUNABLE_SPECIFIER auto hist_score_lmr_constant = TUNABLE_INT("hist_score_lmr_constant", -1147, -2048, -512);
TUNABLE_SPECIFIER auto not_ttpv_lmr_constant = TUNABLE_INT("not_ttpv_lmr_constant", 1067, 512, 2048);
TUNABLE_SPECIFIER auto noisy_lmr_constant = TUNABLE_INT("noisy_lmr_constant", -1145, -2048, -512);
...
TUNABLE_SPECIFIER auto fp_multi = TUNABLE_INT("fp_multi", 199, 100, 300);
...
TUNABLE_SPECIFIER auto quiet_see_prune_multi = TUNABLE_INT("quiet_see_prune_multi", -45, -100, -30);

TUNABLE_SPECIFIER auto asp_window = TUNABLE_INT("asp_window", 19, 10, 50);
...
Score SearchHandler::negamax_step(const Position& old_pos, Score alpha, Score beta, int depth, int ply, uint64_t& node_count, bool is_cut_node) {
    ...
    auto mp = MovePicker(false, tt_move ? entry->get().move() : Move::NULL_MOVE(), search_stack[ply].killer_move, old_pos, history_table, board_hist);
    // move reordering
...
Score SearchHandler::quiescent_search(const Position& old_pos, Score alpha, Score beta, int ply, uint64_t& node_count) {
    ...
    auto mp = MovePicker(!old_pos.in_check(), Move::NULL_MOVE(), search_stack[ply].killer_move, old_pos, history_table, board_hist);
    int total_moves = 0;
    Move best_move = Move::NULL_MOVE();
    std::optional<ScoredMove> opt_move;
    auto found_move = false;
