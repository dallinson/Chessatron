...
Score SearchHandler::quiescent_search(const Position& old_pos, Score alpha, Score beta, int ply, uint64_t& node_count) {
    ...
    if (old_pos.in_check()) {
        ...
        moves = MoveGenerator::generate_legal_moves<MoveGenType::NOISY>(old_pos, old_pos.stm());
    }
    if (moves.size() == 0
        && (old_pos.in_check() || MoveGenerator::generate_legal_moves<MoveGenType::QUIETS>(old_pos, old_pos.stm()).size() == 0)) {
        if (old_pos.in_check()) {
            // if in check
            return ply + MagicNumbers::NegativeInfinity;
        } else {
