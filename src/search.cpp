...
Score SearchHandler::negamax_step(Score alpha, Score beta, int depth, int ply, TranspositionTable& transpositions, int& max_ply, std::array<uint32_t, 8192>& history_table) {

    max_ply = std::max(ply, max_ply);
    ...
    if (depth <= 0 || ply >= MAX_PLY) {
        return quiescent_search<pv_node_type>(alpha, beta, ply, transpositions, max_ply, history_table);
        // return c.evaluate();
    }
    ...
    if (static_eval >= beta && board.get_checkers(board.get_side_to_move()) == 0 && (history.len() == 0 || (history[history.len() - 1].get_move() != Move::NULL_MOVE))) {
        ...
        auto null_score = -negamax_step<pv_node_type>(-beta, -alpha, depth - 1 - (depth >= 8 ? 3 : 2), ply + 1, transpositions, max_ply, history_table);
    ...
    MoveOrdering::reorder_moves(moves, board, tt_entry.get_key() == board.get_zobrist_key() ? tt_entry.get_pv_move() : Move::NULL_MOVE, found_pv_move, history_table, search_stack[ply].killer_moves);
    // move reordering
    ...
    for (size_t evaluated_moves = 0; evaluated_moves < moves.len(); evaluated_moves++) {
        ...
        if (depth >= 2 && is_lmr_potential_move) {
            ...
            score = -negamax_step<NodeTypes::NON_PV_NODE>(-(alpha + 1), -alpha, depth - lmr_reduction + extensions, ply + 1, transpositions, max_ply, history_table);

            // it's possible the LMR score will raise alpha; in this case we re-search with the full depth
            if (score > alpha) {
                score = -negamax_step<NodeTypes::NON_PV_NODE>(-(alpha + 1), -alpha, depth - 1 + extensions, ply + 1, transpositions, max_ply, history_table);
            }
        }
        ...
        if (is_pv_node(node_type) && (!is_lmr_potential_move || score > alpha)) {
            score = -negamax_step<NodeTypes::PV_NODE>(-beta, -alpha, depth - 1 + extensions, ply + 1, transpositions, max_ply, history_table);
        }
...
template <NodeTypes node_type>
Score SearchHandler::quiescent_search(Score alpha, Score beta, int ply, TranspositionTable& transpositions, int& max_ply, std::array<uint32_t, 8192>& history_table) {
    ...
    bool found_pv_move = false;
    MoveOrdering::reorder_moves(moves, board, Move::NULL_MOVE, found_pv_move, history_table, search_stack[ply].killer_moves);
    ...
    for (size_t i = 0; i < moves.len(); i++) {
        ...
        if constexpr (is_pv_node(node_type)) {
            if (evaluated_moves == 0) {
                score = -quiescent_search<NodeTypes::PV_NODE>(-beta, -alpha, ply + 1, transpositions, max_ply, history_table);
            } else {
                score = -quiescent_search<NodeTypes::NON_PV_NODE>(-alpha - 1, -alpha, ply + 1, transpositions, max_ply, history_table);
                if (score > alpha) {
                    score = -quiescent_search<NodeTypes::PV_NODE>(-beta, -alpha, ply + 1, transpositions, max_ply, history_table);
                }
            }
...
Score SearchHandler::run_aspiration_window_search(int depth, Score previous_score, int& max_ply, std::array<uint32_t, 8192>& history_table) {
    Score window = 40;
    Score alpha, beta;
    while (true) {
        if (depth <= 4) {
        ...
        previous_score = negamax_step<NodeTypes::ROOT_NODE>(alpha, beta, depth, 0, table, max_ply, history_table);
...
Move SearchHandler::run_iterative_deepening_search() {
    ...
    search_stack.fill(SearchStackFrame());

    Score current_score = 0;
    for (int depth = 1; depth <= TimeManagement::get_search_depth(tc) && !search_cancelled; depth++) {

        int sel_depth = 0;
        current_score = run_aspiration_window_search(depth, current_score, sel_depth, history_table);
        ...
        if (!search_cancelled && print_info) {
            ...
            printf("info depth %d seldepth %d bestmove %s nodes %" PRIu64 " nps %" PRIu64 " ", depth, sel_depth, pv_move.to_string().c_str(), node_count, nps);
