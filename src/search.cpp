...
Score SearchHandler::quiescent_search(Score alpha, Score beta, TranspositionTable& transpositions, uint64_t& node_count, std::array<uint32_t, 8192>& history_table) {
    ...
    if (moves.len() == 0 && MoveGenerator::generate_legal_moves<MoveGenType::NON_CAPTURES>(board, board.get_side_to_move()).len() == 0) {
        if (board.get_checkers(board.get_side_to_move()) != 0) {
            // if in check
            return MagicNumbers::NegativeInfinity + TimeManagement::get_search_depth(tc);
        } else {
            return 0;
        }
    }
...
Move SearchHandler::run_iterative_deepening_search() {
    ...
    for (int depth = 1; depth <= TimeManagement::get_search_depth(tc) && !search_cancelled; depth++) {
        ...
        if (!search_cancelled && print_info) {
            ...
            if (std::abs(current_score) >= (MagicNumbers::PositiveInfinity - TimeManagement::get_search_depth(tc))) {
